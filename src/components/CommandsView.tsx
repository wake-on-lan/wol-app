...
const CommandsView: React.FC<isDarkModeProps> = ({ isDarkMode }) => {
  ...
  const handleExecuteCommand = async () => {
    ...
    try {
      ...
      if (authMethod === 'password') {
        ...
        const keyToUse =
          selectedKeyFile && selectedKeyFile.uri
  ...
  const handleLoadKey = async (keyId: string) => {
    try {
      ...
      if (savedKey) {
        ...
        setSelectedKeyFile({ name: savedKey.name, uri: '' });
  ...
  const handlePickKeyFile = async () => {
    try {
      const resultPickKey = await pick({
        type: [types.allFiles],
        allowMultiSelection: false,
      });
      if (resultPickKey && resultPickKey.length > 0) {
        const file = resultPickKey[0];
  ...
  const handleSaveCurrentKey = async (nameAppendix = '') => {
    if (!selectedKeyFile || !selectedKeyFile.uri) return null;
    try {
      const keyContent = await readPrivateKeyFile(selectedKeyFile.uri);
      const keyId = await KeystoreService.saveSSHPrivateKey(
        selectedKeyFile.name + nameAppendix,
        keyContent,
      );
      setSelectedKeyId(keyId);
  ...
  const saveCurrentAsConfig = async () => {
    ...
    if (authMethod === 'privateKey' && selectedKeyFile && !selectedKeyId) {
      keyId = await handleSaveCurrentKey(' - Bookmark ' + newConfigName); // use the returned value
    }
  ...
  const applyConfig = async (cfg: SavedConfig) => {
    ...
    if (cfg.authMethod === 'privateKey' && cfg.selectedKeyId) {
      try {
        const savedKey = await KeystoreService.getSSHPrivateKey(
          cfg.selectedKeyId,
        );
        if (savedKey) {
          setPrivateKey(savedKey.privateKey);
        ...
        Alert.alert(
          ...
          e?.message || 'Could not unlock private key.',
        );
      }
    }
